...
def newGame(twoplayer=False):
	...
	class Paddle():
		...
		def updateCam(self, ypos, shoot):
			global screen, white
			self.rect.top = ypos*4.8-20;
...
def gameLoop(paddle, enemy, ball, twoplayer, paddle_two):
	...
	while not topscore > 9:
		xposold = xpos
		yposold = ypos
		ypos = struct.unpack("<L", s.recv(4))[0]
		xpos = struct.unpack("<L", s.recv(4))[0]
		#ypos = ord(s.recv(1));
		#xpos = ord(s.recv(1));
		xpos = xpos>>24
		ypos = ypos>>24
		...
		scoreOldPlayer = scorePlayer
		scoreOldEnemy = scoreEnemy
		scorePlayer = paddle.score
		scoreEnemy = enemy.score 
		
		if (scoreEnemy-scoreOldEnemy):
			streak = 0
		if (scorePlayer-scoreOldPlayer == 1):
			streak += 1
		
		if abs(yposold - ypos) < 3 :
			ypos = yposold 
